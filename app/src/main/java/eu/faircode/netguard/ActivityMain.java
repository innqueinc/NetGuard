...
//        initAds();
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String name) {
        ...
        } else if ("theme".equals(name) || "dark_theme".equals(name)) recreate();
    }
    ...
    private DatabaseHelper.AccessChangedListener accessChangedListener = new DatabaseHelper.AccessChangedListener() {
        ...
        public void onChanged() {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (adapter != null && adapter.isLive()) adapter.notifyDataSetChanged();
                }
            });
        }
    };
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (adapter != null)
                if (intent.hasExtra(EXTRA_CONNECTED) && intent.hasExtra(EXTRA_METERED)) {
                    ivIcon.setImageResource(Util.isNetworkActive(ActivityMain.this) ? R.drawable.ic_security_white_24dp : R.drawable.ic_security_white_24dp_60);
                    if (intent.getBooleanExtra(EXTRA_CONNECTED, false)) {
                        if (intent.getBooleanExtra(EXTRA_METERED, false)) adapter.setMobileActive();
                        else adapter.setWifiActive();
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        searchView.setOnCloseListener(new SearchView.OnCloseListener() {
            ...
            public boolean onClose() {
                ...
                if (adapter != null) adapter.getFilter().filter(null);
                return true;
            }
        });
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if ("uid".equals(sort)) menu.findItem(R.id.menu_sort_uid).setChecked(true);
        else menu.findItem(R.id.menu_sort_name).setChecked(true);
...
//                startActivity(new Intent(this, ActivitySettings.class));
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (Util.batteryOptimizing(this) && getPackageManager().resolveActivity(doze, 0) != null) {
                ...
                if (!prefs.getBoolean("nodoze", false)) {
                    ...
                    dialogDoze = new AlertDialog.Builder(this).setView(view).setCancelable(true).setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
    ...
    private void checkDataSaving() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            final Intent settings = new Intent(Settings.ACTION_IGNORE_BACKGROUND_DATA_RESTRICTIONS_SETTINGS, Uri.parse("package:" + getPackageName()));
    ...
    private void menu_legend() {
        ...
        // Show dialog
        dialogLegend = new AlertDialog.Builder(this).setView(view).setCancelable(true).setOnDismissListener(new DialogInterface.OnDismissListener() {
            @Override
            public void onDismiss(DialogInterface dialogInterface) {
                dialogLegend = null;
            }
    ...
    private void menu_about() {
        ...
        // Show dialog
        dialogAbout = new AlertDialog.Builder(this).setView(view).setCancelable(true).setOnDismissListener(new DialogInterface.OnDismissListener() {
            @Override
            public void onDismiss(DialogInterface dialogInterface) {
                dialogAbout = null;
            }
